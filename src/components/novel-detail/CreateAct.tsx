...
const CreateActPopup: React.FC<CreateActPopupProps> = ({ isOpen, onClose, userId, novelId }) => {
    ...
    const handleSubmit = async () => {
        if (!title.trim()) {
            notifyError('Vui lòng nhập tên Act');
            return;
        }
                            ...
                            >
                                ...
                                <div className="relative">
                                    ...
                                    <motion.label
                                        ...
                                        htmlFor="file-upload"
                                        className={`w-full px-3 py-2 bg-black border border-gray-600 rounded text-white cursor-pointer hover:border-blue-400 transition-colors flex items-center gap-2 ${createActMutation.isPending ? 'opacity-50 cursor-not-allowed' : ''
                                            }`}
                                    >
                                        <Upload size={16} />
