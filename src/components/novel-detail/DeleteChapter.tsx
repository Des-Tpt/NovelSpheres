...
import { X, AlertTriangle, Loader2, FileX } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';

interface DeleteChapterPopupProps {
    isOpen: boolean;
    onClose: () => void;
    chapterData: {
        chapterId: string;
...
const DeleteChapterPopup: React.FC<DeleteChapterPopupProps> = ({ isOpen, onClose, chapterData }) => {
    ...
    const deleteMutation = useMutation({
        ...
        onError: (error: any) => {
            notifyError(error?.message || 'Xóa chapter thất bại!');
        }
    });
                            ...
                            {deleteMutation.isPending && (
                                ...
                                >
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <p className="text-sm text-gray-300">Đang xóa Chapter...</p>
                                    </div>
                                </motion.div>
                            )}
                        </AnimatePresence>
                        ...
                        >
                                ...
                                <div className="flex-1">
                                    <p className="text-gray-300 mb-2">
                                        ...
                                        {chapterData.chapterNumber && (
                                            <span className="font-medium text-orange-400">#{chapterData.chapterNumber}</span>
                                        )}
                                        {chapterData.chapterTitle && (
                                            <>
                                                {chapterData.chapterNumber && ' - '}
                                                <span className="font-medium text-white">"{chapterData.chapterTitle}"</span>
                                            </>
                                        )} không?
                                    </p>
                                    <p className="text-sm text-red-400">
                                        Hành động này không thể hoàn tác và sẽ xóa vĩnh viễn toàn bộ nội dung chapter.
                                    </p>
                                </div>
                            </div>
                            ...
                            >
                                ...
                                <div className="text-sm text-gray-400 space-y-1">
                                    <p>• Toàn bộ nội dung chapter sẽ bị xóa vĩnh viễn</p>
                                    <p>• Số từ và thời gian viết sẽ không thể khôi phục</p>
                                    <p>• Chapter số sẽ không được tự động sắp xếp lại</p>
...
export default DeleteChapterPopup;