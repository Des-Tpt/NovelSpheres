...
const EditActPopup = ({ novelId, act, isOpen, onClose, theme, onUpdate }: EditActPopupProps) => {
    const [title, setTitle] = useState<string>(act.title || '');
    const [actNumber, setActNumber] = useState<number>(act.actNumber || 1);
    const [actType, setActType] = useState<string>(act.actType || 'Hồi');
    ...
    useEffect(() => {
        if (isOpen && act) {
            setTitle(act.title || '');
            setActNumber(act.actNumber || 1);
            setActType(act.actType || 'Hồi');
        }
    }, [isOpen, act]);
    ...
    const handleUpdateAct = async () => {
        setIsPending(true);
        if (!currentUser) {
            notifyError('Vui lòng đăng nhập để sửa act!');
            setIsPending(false);
            return;
        }
        ...
        try {
            ...
            notifySuccess('Cập nhật hồi thành công!');
            onUpdate();
            setTimeout(() => {
                onClose();
            }, 100);
            ...
            notifyError(error?.message || 'Cập nhật act thất bại!');
        } finally {
            setIsPending(false);
        }
    }
    ...
    const hasChanges = act && (title !== act.title || actNumber !== act.actNumber || actType !== act.actType);
                ...
                >
                    ...
                    >
                        ...
                        <div className="flex items-center justify-between px-5 pt-5 pb-1">
                            <div>
                                <h2 className={`text-lg font-bold ${textPrimary} mb-0.5`}>Sửa Thông Tin Hồi</h2>
                                <p className={`text-xs ${textSecondary}`}>
                                    Cập nhật thông tin cho nhóm chương này
                                </p>
                            </div>
                            <button
                                onClick={onClose}
